...
The `i18n` capability manages locale state, translation lookup, language switching, persistence, and exposes a hook/context for components to consume translated strings. It enables the Strengths Navigator app to render all user-facing content in the user's selected language (initially English and Chinese) and provides a foundation for adding further locales without modifying consuming components.
